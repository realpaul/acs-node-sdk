...
Cocoafish.prototype.sendRequest = function(url, method, data, callback, useSecure) {
	...
	if(fileObj) {
		...
		try {
			...
			if(binary) {
				...
				if(authType == utils.oauth) {
					...
					if(this.apiPort != 443 && this.apiPort != 80) {
						message['action'] = protocal + this.apiBaseURL + ":" + this.apiPort + reqURL;
					} else {
				...
				utils.sendRequestWithFile(this.apiBaseURL, this.apiPort, reqURL, apiMethod, data, header, isSecure, callback, this, filePropName, fileName, binary, mimeType);
			} else {
		...
		utils.sendRequest(this.apiBaseURL, this.apiPort, reqURL, apiMethod, data, header, isSecure, callback, this);
	}
};
